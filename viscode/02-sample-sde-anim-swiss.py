...
camera = Camera(fig)
ns = 100
xraw, yraw = torch.linspace(-1.6, 1.6, ns), torch.linspace(-1.6, 1.6, ns)
...
for i, sample in tqdm(enumerate(samples)):
    if (i > 500 and i % 10 == 0) or (i > 800 and i % 5 == 0):
        ...
        _ = plt.imshow(torch.exp(logp).detach().numpy(), extent=(-1.6, 1.6, 1.6, -1.6), cmap="Oranges")
...
animation.save('02-animation-sampling-sde.mp4')
